...
@Configuration
public class AutomaticSkipStatusCommand implements IDiscordCommand {

    private final String COMANDO = "autoSkipStatus";

    private final WolvesvilleController wovController;

    @Autowired
    public AutomaticSkipStatusCommand(WolvesvilleController wolvesvilleController) {
        this.wovController = wolvesvilleController;
    }

    @Override
    public void register(DiscordApi api) {
        SlashCommand command = SlashCommand.with("autoskipstatus", "Altera o modo de skip automático.")
                .createGlobal(api)
                .join();
    ...
    private void respond(SlashCommandCreateEvent event) {
        if (event.getSlashCommandInteraction().getFullCommandName().equalsIgnoreCase(COMANDO)) {
            EmbedBuilder embedBuilder = new EmbedBuilder()
                    .setTitle("Skip automático");

            if (wovController.isAutomaticSkipWaitingTime()) {
                embedBuilder.setDescription("O skip automático do tempo de espera das missões está ATIVADO.");
                embedBuilder.setColor(Color.GREEN);
            } else {
                embedBuilder.setDescription("O skip automático do tempo de espera das missões está DESATIVADO.");
